...
class FirebaseHelper {
  ...
  static final userStream = userType == "Individual" ? _giverStream : _takerStream;

  static final _takerDonationStream = donationCol.where("product", isEqualTo: productType).where("donate", isEqualTo: true).where("complete", isEqualTo: false).snapshots();
  static final _giverDonationStream = donationCol.where("donate", isEqualTo: true).where("complete", isEqualTo: false).snapshots();
  static final donationStream = userType == "Individual" ? _giverDonationStream : _takerDonationStream;

  static final _takerRequirementStream = donationCol.where("product", isEqualTo: productType).where("donate", isEqualTo: false).where("complete", isEqualTo: false).snapshots();
  static final _giverRequirementStream = donationCol.where("donate", isEqualTo: false).where("complete", isEqualTo: false).snapshots();
  static final requirementStream = userType == "Individual" ? _giverRequirementStream : _takerRequirementStream;
